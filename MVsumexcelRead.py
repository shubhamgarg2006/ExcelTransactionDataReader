...
portfolio = {}
purchase_prices = {}  # Track purchase prices for stocks bought today
...
# --- Function to get closing price from Yahoo Finance ---
def get_closing_price(ticker, date):
    """Get closing price for a ticker on a specific date."""
    try:
        # Get data for the date (Yahoo Finance needs next day for proper data)
        ...
        if not hist.empty:
            ...
            print(f"⚠️  No price data for {ticker} on {date.strftime('%Y-%m-%d')}")
            return None
...
for date, day_trades in grouped:
    ...
    # Track stocks bought today for valuation
    bought_today = {}
    ...
    for idx, row in day_trades.iterrows():
        ...
        purchase_price = row.get('TransactionPrice', 0) if 'TransactionPrice' in row and not pd.isna(row['TransactionPrice']) else 0
        ...
        if action == 'buy':
            ...
            portfolio['Cash'] += amount  # amount is negative for buys
            bought_today[ticker] = purchase_price  # Store purchase price
            summary['buys'].append(f"{abs(qty)} shares of {ticker} at ${purchase_price:.2f}/share (Total: ${abs(amount):.2f})")
        elif action == 'sell':
            portfolio[ticker] = portfolio.get(ticker, 0) + qty  # qty negative for sells
            portfolio['Cash'] += amount  # receive sale proceeds
    ...
    for ticker, qty in portfolio.items():
        if ticker != 'Cash' and qty > 0:
            if ticker in bought_today:
                # Use purchase price for stocks bought today
                price = bought_today[ticker]
                value = qty * price
                equity_values[ticker] = {'qty': qty, 'price': price, 'value': value, 'source': 'purchase'}
            else:
                # Use closing price from Yahoo Finance for existing stocks
                price = get_closing_price(ticker, date)
                if price:
                    value = qty * price
                    equity_values[ticker] = {'qty': qty, 'price': price, 'value': value, 'source': 'market'}
                else:
                    equity_values[ticker] = {'qty': qty, 'price': 0, 'value': 0, 'source': 'error'}
            
            total_equity_value += equity_values[ticker]['value']
...
with open(text_output, "w") as f:
    for i, snapshot in enumerate(positions_over_time):
        ...
        # Write positions with valuations
        f.write("POSITIONS AFTER TRANSACTIONS:\n")
        f.write(f"  Cash: ${snapshot['Cash']:.2f}\n")
        for ticker, details in snapshot['equity_values'].items():
            source_label = "purchase price" if details['source'] == 'purchase' else "closing price"
            f.write(f"  {ticker}: {details['qty']} shares @ ${details['price']:.2f} ({source_label}) = ${details['value']:.2f}\n")
        f.write(f"\n  TOTAL EQUITY VALUE: ${snapshot['total_equity_value']:.2f}\n")
        f.write(f"  TOTAL PORTFOLIO VALUE: ${snapshot['Cash'] + snapshot['total_equity_value']:.2f}\n")
        f.write("\n")
...
for i, snapshot in enumerate(positions_over_time):
    ...
    # Add summary section
    excel_rows.append({'Date': date_str, 'Section': 'SUMMARY', 'Field': 'Total Cash Change', 'Value': f"${summary['total_cash_change']:.2f}"})
    ...
    if summary['dividends'] != 0:
        excel_rows.append({'Date': date_str, 'Section': 'SUMMARY', 'Field': 'Dividends', 'Value': f"${summary['dividends']:.2f}"})
    if summary['interest'] != 0:
        excel_rows.append({'Date': date_str, 'Section': 'SUMMARY', 'Field': 'Interest', 'Value': f"${summary['interest']:.2f}"})
    if summary['other'] != 0:
        excel_rows.append({'Date': date_str, 'Section': 'SUMMARY', 'Field': 'Other', 'Value': f"${summary['other']:.2f}"})
    
    # Add positions section
    excel_rows.append({'Date': date_str, 'Section': 'POSITIONS', 'Field': 'Cash', 'Value': f"${snapshot['Cash']:.2f}"})
    for ticker, details in snapshot['equity_values'].items():
        source_label = "purchase" if details['source'] == 'purchase' else "market"
        excel_rows.append({
            'Date': date_str, 
            'Section': 'POSITIONS', 
            'Field': f"{ticker} ({details['qty']} shares @ ${details['price']:.2f} {source_label})",
            'Value': f"${details['value']:.2f}"
        })
    
    # Add totals
    excel_rows.append({'Date': date_str, 'Section': 'POSITIONS', 'Field': 'TOTAL EQUITY VALUE', 'Value': f"${snapshot['total_equity_value']:.2f}"})
    excel_rows.append({'Date': date_str, 'Section': 'POSITIONS', 'Field': 'TOTAL PORTFOLIO VALUE', 'Value': f"${snapshot['Cash'] + snapshot['total_equity_value']:.2f}"})
...
print("\n💰 Equity valuations calculated:")
print("   - Stocks bought today: valued at purchase price")
print("   - Existing stocks: valued at closing price from Yahoo Finance")